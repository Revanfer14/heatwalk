...
Sejak amendemen 2026-08-28 (§Fase 14), Mode 2 mencari sampai satu rute alternatif selain rute teradem dan terpendek (dipangkas same-day dari rencana awal dua alternatif, supaya total kartu di panel maksimum tiga, bukan empat), lewat metode penalti berulang (`web/src/lib/routeAlternatives.ts`) — bukan algoritma k-shortest-path yang terbukti optimal (mis. Yen's algorithm). Metode ini cukup untuk graf jalan skala catchment sekolah dan cepat dihitung client-side, tapi dua konsekuensi mengikutinya: (1) **alternatifnya bisa saja tidak ditemukan sama sekali** — temuan G8 (`docs/METHODOLOGY.md` §Fase 3) sudah menunjukkan ~90% pasangan blok–sekolah punya rute teradem yang identik dengan rute terpendek, jadi jaringan lokal yang padat sering genuinely kehabisan jalur berbeda yang tidak terlalu tumpang tindih; (2) alternatif yang ditemukan adalah alternatif **pertama yang lolos ambang kemiripan** pada percobaan penalti, bukan alternatif paling optimal secara global. Panel dan peta dirancang menerima 2 atau 3 kartu tanpa mematahkan layout — ini bukan bug saat sebuah pasangan asal–sekolah hanya menampilkan rute teradem dan terpendek tanpa alternatif.
