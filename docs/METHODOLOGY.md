...
**Opsi rute majemuk (FR-30).** Selain rute teradem dan rute terpendek, dicari sampai satu rute alternatif lewat metode penalti (`web/src/lib/routeAlternatives.ts`, `ALTERNATE_ROUTE_COUNT = 1` — awalnya 2, dipangkas same-day, lihat §Revisi di bawah): jalankan ulang Dijkstra pada `weight_cool` dengan bobot edge yang sudah dipakai rute-rute lain dikalikan `ALTERNATE_PENALTY_FACTOR = 2.5`, ulangi sampai `MAX_ALTERNATE_ATTEMPTS = 6`, terima kandidat hanya kalau porsi panjangnya yang tumpang tindih dengan rute yang sudah diterima ≤ `MAX_SHARED_LENGTH_RATIO = 0.7`. Ini pencarian jalur k-terbaik yang disederhanakan (bukan Yen's algorithm penuh) — cukup untuk graf jalan skala catchment sekolah (ribuan edge), dan sengaja tidak menjamin alternatif selalu ditemukan: temuan G8 (§Fase 3) sudah menunjukkan ~90% pasangan blok–sekolah punya rute teradem yang identik dengan rute terpendek, jadi banyak jaringan lokal genuinely tidak punya jalur berbeda yang layak. Dijkstra client-side sendiri diganti dari linear-scan O(V²) ke binary heap (`web/src/lib/dijkstra.ts`) supaya menjalankannya sampai tiga kali (teradem, terpendek, satu percobaan alternatif) tetap di bawah <1 detik NFR §7 pada graf 6-8 ribu edge.
