...
### Revisi Fase 6/12 (sesi 30 Agustus): sorotan salah klasifikasi diganti dari garis ke spotlight area

**Pemicu:** Revan melaporkan switch "Walks, should get bus" terlihat seperti mengubah rute, padahal maksudnya menandai zona. Prediket filternya sudah dan selalu murni properti blok (§sesi 28 Agustus di atas) — nol geometri rute. Masalahnya rendering: `useMisclassifiedHighlightLayer.ts` menggambar dua layer `line` (casing 4px `--bg` + garis 2px `--ink`) di atas poligon blok, kosakata cat yang identik dengan garis rute Mode 2, dan blok sensus adalah strip tipis dibatasi jalan — outline-nya menyusuri jalan. Ditambah volumenya besar: dihitung dari `district_blocks.geojson`, Hungerford Elementary punya 222 blok tersorot untuk kategori ini, 150 di antaranya (68%) `kids_est = 0`; Killarney Elementary 186 blok, 110 kosong (59%). Ratusan outline tipis di grid perumahan padat adalah gambar jaringan jalan, dan label switch (hitungan tertimbang-anak) tidak mencerminkan blok kosong yang tetap tersorot di peta — peta dan angka saling bertentangan.

**Perbaikan filter:** `walkShouldBusFilter()` dan `busNotNeededFilter()` di `web/src/lib/misclassifiedHighlight.ts` sekarang mensyaratkan `['>', ['get','kids_est'], 0]` (konstanta `HAS_STUDENTS`), sebelum klausa `status_now`/`class`/`distance_mi` yang sudah ada. Ini bukan perubahan predikat pipeline — `pipeline/summary_build.py:_misclassified_by_hour` menjumlahkan `kids_est`, jadi blok kosong sudah selalu menyumbang 0 ke angka; sekarang ia juga tidak lagi menyumbang tinta. Hungerford turun dari 222 blok tersorot ke 72; Killarney dari 186 ke 76.

**Perbaikan rendering — dari garis ke spotlight area.** Dua layer diubah bersamaan:
- `web/src/hooks/useDoseZoneLayer.ts` menerima `highlightFilter` dan meredupkan choropleth untuk blok yang **tidak** cocok filter aktif — `fill-opacity` turun ke `DIMMED_FILL_OPACITY` (0,06), `line-opacity` outline dan arsiran merah (`HATCH_LAYER_ID`) turun ke `DIMMED_LINE_OPACITY` (0,15). `highlightFilter === null` (kedua switch mati) mengembalikan ekspresi identik dengan sebelum perubahan — nol regresi saat sorotan tidak aktif.
- `web/src/hooks/useMisclassifiedHighlightLayer.ts` sekarang punya layer `fill` (bukan cuma `line`), dicat dengan `buildClassColorExpression` yang sama dipakai choropleth (`--zone-safe`/`--zone-reroute`/`--zone-bus`) pada `HIGHLIGHT_FILL_OPACITY` 0,55, plus garis batas 1,5px `--ink` tanpa casing (casing `--bg` dihapus — itu unsur yang paling membuat sorotan lama terbaca sebagai rute).

**Kenapa layer sorotan butuh `fill` sendiri, bukan cukup meredupkan choropleth:** kategori "Gets bus, doesn't need it" (`busNotNeeded`) secara definisi ada di luar radius kebijakan — `focusedSchoolBlocks` (sumber sorotan) tidak dipotong lingkaran, tapi `policyCircleBlocks` (sumber choropleth) dipotong. Blok-blok itu karena itu tidak pernah ada di data choropleth sama sekali; meredupkan choropleth saja tidak akan menyalakan apa pun untuk kategori ini. Filternya sendiri dipindah dari dihitung dua kali (masing-masing hook memanggil `buildMisclassifiedFilter`) menjadi dihitung sekali di `useDistrictMapLayers.ts` lewat `useMemo`, diteruskan ke kedua hook — satu predikat, dua layer.

**Trade-off FR-22 yang diterima sadar:** FR-22 mensyaratkan interior lingkaran kebijakan tertutup penuh choropleth, tanpa "lubang" abu basemap. Area yang diredupkan ke opacity 0,06 saat sorotan aktif mendekati kesan lubang tersebut. Diterima karena transien dan dipicu pengguna — hanya berlaku selama switch menyala; FR-22 kembali penuh begitu switch dimatikan. Amendemen lengkap ada di `DESIGN.md` §Mode 1 (baris amendemen 2026-08-30).

**Verifikasi:** `npx tsc --noEmit` dan `npm run build` bersih; `npm run lint` (oxlint) nol warning baru dibanding baseline sebelum perubahan; diperiksa manual di `/district` — Hungerford Elementary dengan "Walks, should get bus" menyala menunjukkan area merah pekat jauh lebih kecil dari sebelumnya dan sisa lingkaran meredup tanpa kesan jaring jalan; Hiawassee Elementary jam 08:00 dengan "Gets bus, doesn't need it" menyala (49 anak, di luar lingkaran kebijakan) menunjukkan area hijau terisi di luar choropleth; kedua switch mati mengembalikan choropleth identik dengan sebelum perubahan; FR-16 aktif menyembunyikan sorotan **dan** mengembalikan choropleth ke opacity normal, tidak tertinggal redup.
