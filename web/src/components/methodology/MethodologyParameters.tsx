...
const DETOUR_CAP_MULTIPLIER = 1.4
    ...
    <div className="flex flex-col gap-2">
      <Table>
        <TableBody>
          ...
          <TableRow>
            <TableCell className="text-ink-muted">Walking speed</TableCell>
            <TableCell className="tabular-nums">{WALK_SPEED_MPS} m/s</TableCell>
          </TableRow>
          <TableRow>
            <TableCell className="text-ink-muted">Detour cap</TableCell>
            <TableCell className="tabular-nums">{DETOUR_CAP_MULTIPLIER}× shortest distance</TableCell>
          </TableRow>
          <TableRow>
          ...
          <TableRow>
            <TableCell className="text-ink-muted">Modeled day</TableCell>
            <TableCell className="tabular-nums">{meta.fetched_at}</TableCell>
          </TableRow>
          <TableRow>
            <TableCell className="text-ink-muted">
              Detour penalty (λ), {schools[0]?.name ?? 'this school'}
            </TableCell>
      ...
      <p className="text-xs text-ink-subtle">
        Read live from {schools[0]?.name ?? 'the first analyzed school'}&rsquo;s temps.json. Walking speed and
        detour cap are global pipeline constants; every other row is per-school. λ is calibrated separately per
        school — see docs/METHODOLOGY.md for the full calibration table.
      </p>
    </div>
  )
}
